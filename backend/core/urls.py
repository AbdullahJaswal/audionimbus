...
urlpatterns = [
    # Status
    path(f"{url_prefix}status/", include("status.urls")),
    # Admin
    path(
    ...
    path(f"{url_prefix}audioclip/", include("audioclip.urls")),
]
...
if settings.DEBUG is True:
    ...
    schema_view = get_schema_view(
        openapi.Info(title=os.environ.get("APP_TITLE"), default_version="v1"),
        public=True,
